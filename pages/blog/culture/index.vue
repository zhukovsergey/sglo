<template>
  <div class="container">
    ...
    <h1 class="h1 text-center">Культура</h1>
    <blogthemes />
    <v-row>
...
export default {
  ...
  async asyncData () {
    const { data } = await axios.get('https://xn--48-mlcdei8abd3a7g9b.xn--p1ai/api/blog/culture')
    return { data1: data }
  },
  data: () => ({
    show: false,
  ...
  head () {
    return {
      title: 'Статьи на тему "Культура" Союза женщин Липецкой обалсти',
      meta: [
        {
          hid: 'description',
          name: 'description',
          content:
            'Актуальные новости и статьи на тему Культуры с официального сайта Союза женщин Липецкой области'
        },
