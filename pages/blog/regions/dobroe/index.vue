<template>
  <div class="container">
  <h1 class="text-center">Новости Добровского районного отделения</h1>
...
export default {
  ...
  async asyncData ({ store }) {
    const { data } = await axios.get(
      'https://xn--48-mlcdei8abd3a7g9b.xn--p1ai/api/blog/regions/dobroe'
    )
    return { data1: data }
  },
  mounted () {
    localStorage.setItem('dobroe', JSON.stringify(this.$refs.dobroe.outerText))
  },
  data: () => ({
    show: false,
    pagination: '',
  ...
  head () {
    return {
      title:
        'Статьи Добровского регионального отделения Союза женщин',
      meta: [
        {
          hid: 'description',
          name: 'description',
          content:
            'Актуальные новости и статьи Добровского регионального отделения с официального сайта Союза женщин Липецкой области'
        },
