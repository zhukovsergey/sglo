<template>
  <div class="container">
    ...
    <h1 class="h1 text-center">Волна памяти</h1>
    <blogthemes />
    <v-row>
...
export default {
  ...
  async asyncData () {
    const { data } = await axios.get('https://xn--48-mlcdei8abd3a7g9b.xn--p1ai/api/blog/waveofmemory')
    return { data1: data }
  },
  data: () => ({
    show: false,
  ...
  head () {
    return {
      title: 'Статьи на тему "Волна памяти" Союза женщин Липецкой обалсти',
      meta: [
        {
          hid: 'description',
          name: 'description',
          content:
            'Актуальные новости и статьи на тему "@"Волна памяти" с официального сайта Союза женщин Липецкой области'
        },
